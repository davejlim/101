...
def initialize_deck
  deck = []
  deck = (SUITS.keys).product(VALUES.keys) do |suit, value|
...
def joinand(arr, sep = ', ', wrd = 'and ')
  n = arr.size - 1
  arr.slice(0, n).join(sep) + sep + wrd + arr.last
end
...
def translate_cards(who)
  ...
  if who.flatten.size > 2
    who.each do |card|
      translated << "#{card[:value]} of #{SUITS[card[:suit]]}"
    end
...
def display_winner(plr, dlr, wnr)
  if wnr == 'player'
...
loop do
  ...
  loop do
    ...
    case display_winner(player_cards, dealer_cards, winner)
    when 'player'
      player_score += 1
    when 'dealer'
      dealer_score += 1
