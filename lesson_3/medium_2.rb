...
def fun_with_ids
  ...
  1.times do
    ...
    puts "a_outer id was #{a_outer_id} before the block and is: #{a_outer_inner_id} inside the block." # same as a_outer
    ...
    puts "d_outer inside after reassignment is #{d_outer} with an id of: #{d_outer_id} before and: #{d_outer.object_id} after." # same object_id as a_outer
    puts
    ...
    puts "d_inner is #{d_inner} with an id of: #{d_inner_id} inside the block (compared to #{d_outer.object_id} for outer)." # same object_id as previous which sould be the same as a_inner
    puts
  end
